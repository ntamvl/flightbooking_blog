...
if($categories){
	foreach($categories as $category) {
		$cats.= '<a href="'.get_category_link( $category->term_id ).'" title="' . esc_attr( sprintf( __( "Xem tất cả bài viết trong %s", 'theme' ), $category->name ) ) . '">'.$category->cat_name.'</a>'.$separator;
	}
}
...
function mom_related_posts () { ?>
                <h2 class="single-title"><?php _e('Bài viết liên quan', 'theme'); ?></h2>
