...
'ignore_sticky_posts' => 1,
		...
		<?php if (mom_post_image() != false) { if ($unique_posts) {$do_unique_posts[] = get_the_ID();} ?>
                       <li <?php post_class(); ?> itemscope itemtype="http://schema.org/Article">
                            <?php mom_post_image_full('small-wide'); ?>
                        </li>
		<?php } ?>
		<?php endwhile; else: ?>
		<?php endif; ?>
		...
		<?php if ( $query->have_posts() ) : while ( $query->have_posts() ) : $query->the_post(); if ($unique_posts) {$do_unique_posts[] = get_the_ID();} ?>
				...
				<?php if (mom_post_image() != false) { ?>
                                   <div class="sb-item-img">
                                        <a href="<?php the_permalink(); ?>"><?php mom_post_image_full('scrolling-box', 'big-wide-img'); ?></a><span class="post-format-icon"></span>
                                   </div>
				<?php } ?>
...
if (in_array('both-sidebars',$classes)) {
	...
	?>
		<!--News box 2 columns-->
                <div class="news-box base-box nb-style2 nb-2col <?php echo $last.$wide_class; ?>">
                    <header class="nb-header" <?php echo $header_background; ?>>
                        <h2 class="nb-title" <?php echo $header_text_color; ?>><?php echo $title_holder; ?></h2>
                    </header> <!--nb header-->
                    <div class="nb-content">
...
?>
		...
		<?php if ( $query->have_posts() ) : while ( $query->have_posts() ) : $query->the_post(); if ($unique_posts) {$do_unique_posts[] = get_the_ID();} ?>
				<?php if (mom_post_image() != false) { ?>
                                <a href="<?php the_permalink(); ?>" data-tooltip="<?php the_title(); ?>" class="simptip-position-top simptip-movable"><?php mom_post_image_full($img_size, 'big-wide-img'); ?></a>
				<?php } ?>
		<?php endwhile; else: ?>
		<?php endif; ?>
		<?php wp_reset_postdata(); ?>
